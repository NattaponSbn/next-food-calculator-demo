...
import { AxiosInstance } from "axios"; // Import type ของ Axios
import { SuccessResponse } from "../../models/shared/common.model";
import { MasterIngredientGroupItemsModel, MasterIngredientGroupResponseItemModel, MasterIngredientGrouRequestItemModel } from "../../models/master/ingredient-group/ingredient-group.mode";
...
export const ingredientGroupService = {
  ...
  create: async (createData: MasterIngredientGrouRequestItemModel): Promise<SuccessResponse> => {
    const result = await apiClient.post<SuccessResponse>(RESOURCE_PATH, createData);
    return result.data!;
  },

  update: async (id: number, updateData: MasterIngredientGrouRequestItemModel): Promise<SuccessResponse> => {
