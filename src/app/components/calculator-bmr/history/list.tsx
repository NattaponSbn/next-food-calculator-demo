...
import { CalculatorBMRItemsModel, CalculatorBMRRequestModel } from '@/app/core/models/calculator-bmr/bmr.model';

const CalculatorBMRHistoryListPage = () => {
  ...
  const columns = useMemo<ColumnDef<CalculatorBMRItemsModel>[]>(
    () => [
      {
        id: 'no',
        header: t('system.no'),
  ...
  const initialCriteria = useMemo(() => new CalculatorBMRRequestModel(), []);
  ...
  const realFetchFn = useCallback((request: ApiSearchRequest) => {
    ...
    return calculatorBMRService.search(request);
  }, [calculatorBMRService]);
  ...
  const { table, isLoading, refetch } = useServerSideTable<CalculatorBMRItemsModel>({
  ...
  const handleCreate = async () => {
    try {
      ...
      router.replace(`/ui/calculator-history-bmr/manage?mode=${modeType.create}`);
    } catch (error) {
      console.info('การสร้างถูกยกเลิก');
    }
  };

  const handleEdit = async (item: CalculatorBMRItemsModel) => {
    try {
      // เรียกใช้โหมด 'edit' พร้อมส่งข้อมูลเริ่มต้น
      router.replace(`/ui/calculator-history-bmr/manage?mode=${modeType.edit}&id=${item.id}`);
    } catch (error) {
      console.info('การแก้ไขถูกยกเลิก');
    }
  };

  const handleView = (item: CalculatorBMRItemsModel) => {
    // โหมด View ไม่จำเป็นต้อง await เพราะเรามักจะไม่สนใจผลลัพธ์
    router.replace(`/ui/calculator-history-bmr/manage?mode=${modeType.view}&id=${item.id}`);
  };


  const handleDelete = async (item: CalculatorBMRItemsModel) => {
    try {
      ...
      if (result.isConfirmed) {
        console.log(`กำลังลบ ${item.peopleName}...`);
        const result = await calculatorBMRService.delete(item.id);
        if (!result) return;
        // เมื่อลบสำเร็จ อาจจะแสดง Alert อีกอัน
        showSuccessAlert('ลบสำเร็จ!', `${item.peopleName} ถูกลบแล้ว`);
...
export default CalculatorBMRHistoryListPage;