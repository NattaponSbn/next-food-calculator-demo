...
import { ActivitysModal, ActivitysModalProps } from './modals/editor-activity-modal';
import { activityService } from '@/app/core/services/master/activity.service';
import { MasterActivityItemsModel, MasterActivityRequestModel } from '@/app/core/models/master/activity/activity.model';

const MasterActivitysList = () => {
  const { t } = useTranslation();
  const showUnitsModal = useModal<ActivitysModalProps, any>(ActivitysModal);
  ...
  const columns = useMemo<ColumnDef<MasterActivityItemsModel>[]>(
    () => [
      {
        id: 'no',
        header: t('system.no'),
      ...
      },
      {
        ...
        header: ({ column }) => (
          <FilterControl
            column={column}
            title={t('master.atvyCode')}
            placeholder={t('master.atvyCode')}
        ...
        header: ({ column }) => (
          ...
          <div className="flex flex-col items-center justify-center gap-2">
            ...
            <SortableHeader column={column}>
              {`${t('master.atvyName')} (${t('system.language.en')})`}
            </SortableHeader>

            {/* ส่วนที่ 2: Filter Control */}
            <FilterControl
              column={column}
              placeholder={`${t('master.atvyName')} (${t('system.language.en')})`}
              meta={{ filterType: 'text' }} // <-- กำหนด filter type
              onFilterIconClick={handleOpenFilter}
            />
          </div>
      ...
      },
      {
        ...
        header: ({ column }) => (
          <div className="flex flex-col items-center justify-center gap-2">
            ...
            {/* ส่วนที่ 1: Title และปุ่ม Sort */}
            <SortableHeader column={column}>
              {`${t('master.atvyName')} (${t('system.language.th')})`}
            </SortableHeader>

            {/* ส่วนที่ 2: Filter Control */}
            <FilterControl
              column={column}
              placeholder={`${t('master.atvyName')} (${t('system.language.th')})`}
              meta={{ filterType: 'text' }} // <-- กำหนด filter type
              onFilterIconClick={handleOpenFilter}
            />
          </div>
  ...
  const initialCriteria = useMemo(() => new MasterActivityRequestModel(), []);
  ...
  const { table, isLoading, refetch } = useServerSideTable<MasterActivityItemsModel>({
  ...
  const handleEdit = async (item: MasterActivityItemsModel) => {
    try {
  ...
  const handleView = (item: MasterActivityItemsModel) => {
  ...
  const handleDelete = async (item: MasterActivityItemsModel) => {
    try {
      ...
      if (result.isConfirmed) {
        const result = await activityService.delete(item.id);
        if (!result) return;
      ...
      <div className="panel-header">
        <div className="flex items-center gap-2">
          <h2 className="panel-title">
            {t('master.atvyList')} {/* หรือชื่อหัวตาราง */}
          </h2>
...
export default MasterActivitysList;